 ...
 *
 *  描述：封装对Access数据库的各类操作，将要执行的SQL语句直接传入方法即可返回结果。
 *        支持简单SQL语句和带参数的SQL语句。
 *  作者：杨慧炯
 *  版本：V1.0
 *  创建时间：2020-02-10
 *  类名：Access数据库操作抽象类DbHelperAccess
 *  
    ...
    public abstract class DbHelperAccess
    {
        ...
        public static int ExecuteSql(string SQLString, string content)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                ...
                System.Data.OleDb.OleDbParameter myParameter = new OleDbParameter("@content",OleDbType.VarWChar);
